...
@app.get("/health")
def health_check():
    """Health check endpoint for deployment monitoring - Always returns healthy"""
    data_exists = COUNTIES_FILE.exists()
    enhanced_exists = ENHANCED_COUNTIES_FILE.exists()

    # Always return 200 OK with "healthy" status for Coolify compatibility
    # Data loading is optional - app can run without data and data can be added later
    return JSONResponse(
        status_code=200,
        content={
            "status": "healthy",  # Always healthy, even without data
            "app_running": True,
            "data_loaded": data_exists,
            "enhanced_data_loaded": enhanced_exists,
            "message": "Data can be generated by running scripts/download_noaa_hurricanes.py" if not data_exists else "All systems operational"
        }
    )
